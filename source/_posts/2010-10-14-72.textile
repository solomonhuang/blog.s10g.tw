...
建立SVN目錄
<pre class="brush: text"># mkdir -p /svn/repos{% endcodeblock %}
建立SVN/TRAC帳號
<pre class="brush: text"># touch /svn/svn-auth-file
...
建立SVN檔案庫
<pre class="brush: text"># svnadmin create proj01{% endcodeblock %}
變更目錄權限，提供httpd讀寫的權限
<pre class="brush: text"># chown apache:apache /svn -R{% endcodeblock %}
設定啟動httpd
<pre class="brush: text"># chkconfig httpd on
# service httpd start{% endcodeblock %}
停用防火牆與SELinux或是正確地設定
<pre class="brush: text"># system-config-securitylevel{% endcodeblock %}
